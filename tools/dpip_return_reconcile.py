...
merely because the assurance proposition remains unresolved. This reconciler
turns a machine-readable DPIP return into one finite RAHP transition:

* PASS / NOT_APPLICABLE -> resolved
* FAIL -> finding-open
* INDETERMINATE -> evidence-required
* unknown/unparseable -> review-required

For non-resolved outcomes it creates or reuses a durable RAHP residual issue,
records an idempotent reconciliation marker on the referral, and closes the
completed referral container. Closing the referral is never represented as a
privacy PASS.
"""
...
def parse_return(comments: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Return the newest attributable DPIP return and its structured disposition."""
...
def reconciliation_state(conclusion: str) -> str:
...
{returned.get('residual') or 'The returned specialist result is not sufficient to close the assurance proposition; only the explicitly required evidence/remediation can change this residual state.'}
...
def reconcile_one(repo: str, number: int, token: str) -> str:
    ...
    state = reconciliation_state(returned["conclusion"])
    ...
    lines += [
        "",
        "The specialist handoff is complete. Closing this referral container does **not** convert FAIL or INDETERMINATE to PASS; unresolved work is carried by the durable residual above.",
    ]
    if not referral:
        lines += [
            "",
            "This issue is not recognizably a referral-only container, so it is left open for explicit RAHP review rather than being auto-closed.",
        ]
...
def self_test() -> int:
    sample = [{
        "body": """<!-- dpip-return:sankarshanmukhopadhyay/dtg-privacy-implementation-profile#147 -->
```yaml
dpip_disposition:
...
""",
    ...
    assert reconciliation_state("unexpected") == "review-required"
